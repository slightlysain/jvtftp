...
public final class JvtftpFromNetASCIIOutputStream extends FilterOutputStream {
	private boolean __lastWasCR;
	private byte[] newLineBytes = { '\n' };

	// private System.getProperty("line.seprator");
	 ...
	 ***/
	public JvtftpFromNetASCIIOutputStream(OutputStream output) {
		super(output);
		__lastWasCR = false;
	}
	...
	private void __write(int ch) throws IOException {
		...
		if (__lastWasCR) {
			...
			case '\n':
				...
				break;
			default:
				throw new IOException("Unexpected characters order in stream");
			}
			__lastWasCR = false;
			return;
		} else if ('\r' == ch) {
